...
""",
    ...
    SpringTopic(
        ...
        summary=(
            ...
            "면접에서는 각 계층이 무엇을 하면 안 되는지도 함께 설명하는 것이 중요합니다.",
        ),
        details=(
            "Controller는 HTTP 요청/응답, 파라미터 바인딩, 입력 검증을 담당합니다.",
            "Service는 하나의 유스케이스를 처리하며 트랜잭션 경계와 비즈니스 규칙을 담당합니다.",
            "Repository는 DB 접근을 추상화하고 쿼리나 영속성 기술에 대한 세부 사항을 감춥니다.",
        ),
        implementation=(
            "Controller에서 비즈니스 로직이나 JPA 쿼리를 직접 다루지 않습니다.",
            "Service 메서드는 유스케이스 단위로 이름을 짓고, 필요한 경우 `@Transactional`을 둡니다.",
            "Repository는 조회 조건과 저장/삭제 같은 데이터 접근 책임에 집중합니다.",
            "테스트는 Service 단위 테스트와 Repository 통합 테스트를 목적에 맞게 나눕니다.",
        ),
        questions=(
        ...
        answer=(
            "Controller, Service, Repository를 나누는 이유는 각 코드의 변경 이유를 분리하기 위해서입니다. "
            "Controller는 HTTP와 입력 검증, Service는 비즈니스 유스케이스와 트랜잭션, Repository는 데이터 접근을 담당합니다. "
            "이렇게 분리하면 API 형식이 바뀌어도 핵심 비즈니스 로직의 영향을 줄일 수 있고, DB 접근 방식이 바뀌어도 Controller까지 변경이 번지지 않게 만들 수 있습니다."
        ),
